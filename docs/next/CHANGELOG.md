...
- The sidebar is now one stable Spaces → tab/window tree, including agentless tabs. Every tab renders as one status-first title row even when it owns multiple panes; priority configuration remains compatible but cannot reorder identity rows, and completed tabs stay `done` until opened.
