...
### Changed
- Desktop sessions now show a theme-derived, right-aligned native status row containing the focused folder and Git branch, device name, Herdr version, CPU, and memory. The left side stays blank; the row does not collect or display IP addresses, transfer rates, usernames, battery, date, or time. Set `ui.status_bar.enabled = false` to hide the row and stop its background sampling.
