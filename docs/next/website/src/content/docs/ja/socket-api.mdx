...
展開・折りたたみサイドバーとモバイルのナビゲーションでは Spaces ツリーを 1 つの
フラットなプロジェクションに置き換え、そのマウスターゲットと番号付きフォーカスを
制御します。設定した前後のエージェント移動アクションは、引き続きワークスペース・
タブ・ペインの正規の順序（フィルタなし）を使います。このプロジェクションは
`agent.list`、通知、検出、グローバルな要対応件数を変更しません。
