...
| ループ | `loop.list`、`loop.run_history` |
...
`loop.run_history` は読み取り専用です。`loop_id` を省略すると、レシートから観測されたすべての実行を返します。1 つのループ id を渡すと、レシートにループ識別子がある場合、その識別子を持つレコードだけを返します。レシートスキーマの移行中は、どの実行にもループ識別子がなければフィルターしません。レスポンスには、選択した `loop_id`、型付きの `runs`、不正なレシート行の数を示す `skipped_lines` が含まれます。読み取っても TUI の詳細ビューは開かず、状態も変えません。
...
レスポンスには `workspace_id`、`tab_id`、`zoomed`、`focused_pane_id`、`root` が含まれます。`root` は `pane` ノードと `split` ノードの BSP ツリーです。ペインノードは `pane_id`、`label`、`cwd`、argv の `command` を含められます。分割ノードは `direction` (`left`、`right`、`up`、`down`)、`ratio`、`first`、`second` を使います。
...
`agent.view.set` は、一つの一時的な宣言型互換プロジェクションを設定します。
エージェントの情報や現在の UI コンテキストが変わるたびに再評価され、番号付き Agent
フォーカスの順序を制御します。表示されるデスクトップ、折りたたみ、モバイルの
サイドバーは、正規の Spaces → タブ → ペイン所有関係ツリーのままです。この
プロジェクションは、その行やマウスターゲットの絞り込み・並べ替えを行いません。
設定した前後のエージェント移動アクションも、フィルタなしの正規のワークスペース・
タブ・ペイン順を使います。`agent.list`、通知、検出、グローバルな要対応件数も
変更しません。

現在表示中の Space にいるエージェント、または別の場所で対応が必要なエージェントを
含め、要対応度と直近の状態遷移で並べ替える番号付きフォーカスプロジェクションの例です:
...
ループ履歴の購読には `loop.run_history_updated` を使います。レシートに基づく履歴が変わると Herdr がこのイベントを発行します。イベントには `loop_id`、現在の型付き `runs`、`skipped_lines` が含まれ、読み取り専用の履歴レスポンスと同じ形です。
