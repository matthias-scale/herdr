...
| アクション | キー |
| --- | --- |
| 全 Space の次 / 前のタブ | `prefix+n` / `prefix+p` |
| この Space の次 / 前のタブ | `prefix+ctrl+n` / `prefix+ctrl+p` |
...
| Goto ピッカー | `prefix+g` |
| サイドバーの表示切り替え | `prefix+shift+b` |
| ループ実行履歴を開く | `prefix+ctrl+h` |
| 読み取り専用の Symphony ダッシュボードを開く | `prefix+shift+s` |
| フリートの Runs セクションを表示 | `prefix+alt+r` |

ループ実行履歴ビューには、すべてのループのレシートに記録された実行が、結果、ゲート、人による操作回数、所要時間とともに表示されます。レシートファイルが変わると自動更新されます。Esc または `prefix+ctrl+h` をもう一度押すと閉じます。

Symphony ダッシュボードは、実行中の Temporal ワークフローについて名前、フェーズ、名前付き待機、経過時間を表示します。Enter を押すと、対応するチェックアウトでワークフロー、チケット、プルリクエスト、レシートのコンテキストを持つ対話型 Herdr ウィンドウが開きます。ダッシュボードからワークフローの signal、cancel、merge は実行できません。

Runs セクションは `ra-launch` と `ra-wrap` の実行をフリートのマシン別に表示します。実行をクリックすると、その `out.log` をターミナルタブで追跡します。
...
previous_window = "prefix+p" # 全 Space の前のタブ
next_window = "prefix+n" # 全 Space の次のタブ
next_blocked_window = "prefix+b" # 全 Space の次の blocked タブ
...
4 方向の分割には衝突しないデフォルトがあります。`prefix+v` は右、`prefix+shift+v` は左、`prefix+minus` は下、`prefix+shift+minus` は上です。従来の `split_vertical` と `split_horizontal` は引き続き右と下を表します。`next_window` と `previous_window` は、エージェントのないタブも含むすべてのタブを、ワークスペース順、その中のタブ順で循環し、両端でラップします。サイドバーや Space の折りたたみ状態は順序に影響しません。`next_tab` と `previous_tab` はアクティブな Space 内だけを移動します。

`next_blocked_window` は同じグローバル順序を前に進み、ペイン状態が `blocked` に集約されるタブだけで停止します。最後の一致の後は先頭に戻ります。blocked なタブがなければ、フォーカスは移動しません。デフォルトは `prefix+b` です。`toggle_sidebar` は `prefix+shift+b` を使います。

キーバインドなしでも、`herdr window next` と `herdr window previous` で同じグローバル循環を使えます。どちらも両端でラップします。
