...
サイドバーは Herdr のメインダッシュボードです。デフォルトの `spaces` 並び順では、1 つのワークスペースツリーとして表示されます。各ワークスペースはロールアップ行を保持し、その配下に検出済みのエージェントを展開できます。リンクされた worktree の展開とエージェントの展開は独立しています。コンパクト表示とモバイル表示でも、同じ所有関係のツリーが保たれます。

`ui.agent_panel_sort = "priority"` を設定すると、ツリーは要対応度順のフラットな Agent プロジェクション 1 つに置き換わります。一時的な `agent.view.set` クエリも同様に、ラベル付きのフィルタ済みプロジェクション 1 つに置き換えます。ワークスペースピッカーを開くと、どちらのプロジェクションも解除せずに一時的に Spaces ツリーへ戻ります。サイズ、折りたたみモード、並び順、マウスの挙動、ペインの境界線、その他の表示設定については、[設定リファレンス](/docs/config-reference/)で `ui.` を検索してください。

展開された各 Agent ウィンドウ行の末尾には、コンパクトなアクティビティ時間が表示されます。作業中のウィンドウでは現在のアクティブ区間の継続時間、アイドル中のウィンドウでは直近のアクティブ区間が終了してからの経過時間を示します。固定幅のフィールドは表示値が変わる時だけ更新されるためラベル位置が安定し、サイドバーが狭い場合はウィンドウ情報より先に省略されます。
...
デスクトップレイアウトでは、タブバーの上に右寄せの全幅ステータス行が表示されます。コンパクトな情報順は、フォーカス中のフォルダー、Git ブランチ、デバイス名、CPU、メモリです。左側は空白のままです。デスクトップ幅では、`CPU` と `MEM` のラベル付きセグメントが表示され、単位にはパーセントと GiB が使われます。サンプルを利用できない場合は `CPU --%` または `MEM --/-- GiB` と表示されます。この行はネットワーク情報やユーザー名を表示もサンプリングもせず、Herdr バージョン、バッテリー、日付、時刻も表示しません。ターミナルが狭くなると、フォルダー、Git ブランチ、デバイス名の順に省略されます。
...
`row_gap` は、Space のルート間、および Agent の子やフラットな Agent エントリ間に入れる空白行数を指定します。Agent と Space の値は引き続き独立しています。デフォルトは `0` で、エントリを詰めて表示します。以前の間隔に戻すには `1` に設定します。`rows` で定義したコンテンツ行の間隔には影響しません。連続するインデントされた worktree の子は、1 つの Space グループとして詰めて表示されます。
...
サイドバーの行設定は、展開されたデスクトップサイドバーにだけ適用されます。折りたたみ表示とモバイル表示は、同じ所有関係と展開状態のツリーをコンパクトに描画します。
