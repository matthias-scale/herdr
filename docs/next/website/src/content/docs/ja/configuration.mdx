...
デスクトップレイアウトでは、タブバーの上に全幅のステータス行が表示され、フォーカス中のセッション識別情報、ディレクトリ、Git ブランチ、利用可能なネットワーク情報とバッテリー情報、日付、時刻が示されます。デスクトップ幅では、`MEM` と `CPU` のラベル付きセグメントが常に表示され、単位には GiB とパーセントが使われます。サンプルを利用できない場合は `MEM --/-- GiB` または `CPU --%` と表示されます。ターミナルが狭くなると、時刻、日付、バッテリー、IP アドレス、帯域幅、Git ブランチ、ディレクトリ、ユーザー、ホストの順に省略されます。
