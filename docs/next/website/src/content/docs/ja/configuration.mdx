...
サンプラーは直ちに開始し、同時に実行するサンプルを 1 つまでに制限しながら 2 秒ごとに更新します。失敗した場合は次の間隔で再試行します。4 秒に達した古いサンプルは破棄され、利用不可の値が表示されます。プラットフォームメトリクスには、ネイティブ API またはローカルの proc/sys ファイルだけが使われます。
