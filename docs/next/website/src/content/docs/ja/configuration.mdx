...
`ui.agent_panel_sort = "priority"` は互換性と注意サマリーのために引き続き受け付けられますが、最終的なサイドバーは常に標準のワークスペース、タブ、ペイン順を保ちます。ライフサイクルの変化や一時的な `agent.view.set` クエリで所有関係行が並び替えられることはありません。サイズ、折りたたみモード、マウス操作、ペイン境界線などは、[設定リファレンス](/docs/config-reference/)で `ui.` を検索してください。
