...
デスクトップレイアウトでは、タブバーの上に右寄せの全幅ステータス行が表示されます。コンパクトな情報順は、フォーカス中のフォルダー、Git ブランチ、デバイス名、Herdr バージョン、CPU、メモリです。コンテキストをまとめ、その後にデバイス、ビルド、リソース情報を並べます。左側は空白のままです。デスクトップ幅では、`CPU` と `MEM` のラベル付きセグメントが表示され、単位にはパーセントと GiB が使われます。サンプルを利用できない場合は `CPU --%` または `MEM --/-- GiB` と表示されます。この行は IP アドレス、ダウンロード／アップロード速度、ユーザー名、バッテリー、日付、時刻を表示もサンプリングもしません。ターミナルが狭くなると、フォルダー、Git ブランチ、デバイス名、Herdr バージョンの順に省略されます。
