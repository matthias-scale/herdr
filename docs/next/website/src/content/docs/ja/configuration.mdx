...
サイドバーは Herdr のメインダッシュボードです。デフォルトの `spaces` 並び順は、ワークスペース→タブ→ペインの 1 つの所有関係ツリーです。エージェントが検出されていないタブも含め、すべてのタブとペインがレイアウト順に表示されます。リンクされた worktree とペインの展開は独立し、コンパクト表示とモバイル表示でも同じツリーが保たれます。選択は強い背景色ではなく、前景色と太字で示されます。
...
展開された各タブは保存済みのタイトルを 1 回だけ表示し、その下にペイン行が並びます。ペインの状態は `working`、`blocked`、`done` として明示されます。完了済みのペインを開くと、タブの順序を変えずに `done` が解除されます。アクティビティ経過時間は補助情報であり、未確認の `done` 状態を置き換えません。
...
`row_gap` は、Space のルート間とペインの子の間に入れる空白行数を指定します。Agent と Space の値は引き続き独立しています。デフォルトは `0` で、エントリを詰めて表示します。以前の間隔に戻すには `1` に設定します。`rows` で定義したコンテンツ行の間隔には影響しません。連続するインデントされた worktree の子は、1 つの Space グループとして詰めて表示されます。
