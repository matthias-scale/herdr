...
1 つの認識済みプロバイダーに紐づく行では、タイトル直後に短い識別子が付きます。Claude は `· cc`、Codex は `· cx`、Pi は `· pi` です。エージェントなし、未対応、複数プロバイダー混在、エージェント終了後の行では識別子を表示しません。Codex がバックグラウンドターミナルを明示的に報告した場合、0 でない件数が識別子の後に `N >_` として表示されます。この件数はタブ内のペイン間で合計され、ライフサイクル状態や並び順には影響しません。最新通信からの経過時間は右寄せのままです。
