...
`idle` は、エージェントが入力待ちで、そのペインがフォーカス中の Herdr UI ですでに開かれたことを示します。`done` は同じ基礎的な idle 状態ですが、バックグラウンド作業が完了してから、そのエージェントのペインを開くか `pane focus` / `agent focus` で対象にするまで使われます。同じタブ内の別のペインを開いたり、CLI で読み取ったりするだけでは表示済みになりません。`blocked` は承認または質問 UI を Herdr が認識した状態です。`unknown` はエージェントが存在するもののライフサイクルを確実に分類できない状態で、成功完了を意味しません。違いが重要なら正確な `--until` を指定してください。
