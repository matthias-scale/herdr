...
`[[remote.fleet.hosts]]` のエントリーを追加すると、そのホストが Hosts ドック
サーフェスに表示されます。herdr は到達性、バージョン、エージェントの一覧を
バックグラウンドで更新します。到達可能なリモートホストを選択すると、新しい
ローカルペインで `herdr --remote` が開きます。
...
省略可能な `session` の値は、ホストを開くときに `--session` を追加します。
`local = true` を指定したホストは、一覧の取得にローカルの API ソケットを使い、
SSH でのポーリングは行いません。Hosts サーフェスが空のときは
`[[remote.fleet.hosts]]` を案内します。
