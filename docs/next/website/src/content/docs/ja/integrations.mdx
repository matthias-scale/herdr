...
Claude Code と Codex の連携は、各 `UserPromptSubmit` でプロバイダーがターンを開始する前に Herdr のエージェントタイトルを再計算します。両方のアダプターは同じ決定論的エンジンを使うため、追加のモデル呼び出しやプロンプトコンテキストはありません。タイトルは 3〜7 語、表示文字数 48 文字以内で、制御文字、端末エスケープ、パス、ユーザー識別子、機密情報らしい値を除去します。
