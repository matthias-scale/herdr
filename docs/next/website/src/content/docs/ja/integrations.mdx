...
Claude Code と Codex の連携は、各 `UserPromptSubmit` でプロバイダーがターンを開始する前に、所有するタブの Herdr タイトルを 1 つだけ再計算します。自動作業タイトルには Claude Code と Codex の両方で**インテグレーションバージョン 8**が必要です。`herdr integration status` を実行し、古いと報告されたインテグレーションを再インストールしてください。各フックはそのペインを所有する Herdr バイナリを呼び出し、両方のアダプターは同じ決定論的エンジンを使うため、追加のモデル呼び出しやプロンプトコンテキストはありません。タイトルは 1〜7 語、表示文字数 48 文字以内です。エンジンは制御文字、端末エスケープ、パス、ユーザー識別子、機密情報らしい値を除去し、末尾側で最後に見つかった意味のある段落から具体的な目的を導き出します。短い続行指示やメタ指示だけの続行では、そのペイン、プロバイダー、ネイティブセッション固有の最初の依頼内容を維持します。

更新はペイン、プロバイダー、ネイティブセッション ID、単調増加シーケンスに結び付けられます。同じ正規化済みタイトルは書き込まず、古いターンやセッションの遅延更新は新しいタイトルを上書きできません。必要なフック、ソケット、ペイン、またはサブエージェント専用ペインがない場合は安全に何もしません。
...
Herdr はデフォルトで `~/.grok` を使い、`GROK_HOME` が設定されていればそちらを使います。Grok の設定ディレクトリはあらかじめ存在している必要があります。Grok はそのディレクトリ内のすべての `hooks/*.json` ファイルを統合するため、インストールは Herdr の `SessionStart` エントリを含む自己完結型の `hooks/herdr.json` を `hooks/herdr-agent-state.sh` スクリプトの隣に書き込み、他のフックファイルは編集しません。アンインストールは Herdr が所有するこの 2 ファイルだけを削除します。
