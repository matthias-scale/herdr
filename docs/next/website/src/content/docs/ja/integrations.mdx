...
Claude Code と Codex のインテグレーションは、プロバイダーがターンを始める前に、`UserPromptSubmit` ごとに所有タブの 1 つの Herdr タイトルを再計算します。自動ワークタイトルには、Claude Code と Codex の両方でインテグレーションバージョン **8** が必要です。`herdr integration status` を実行し、古いと報告されたインテグレーションを再インストールしてください。各フックはペインを所有する Herdr バイナリを呼び出し、両アダプターは同じ決定的なタイトルエンジンを使います。追加のモデルリクエストやプロンプトコンテキストはありません。ワークタイトルはライフサイクル権威、ネイティブモデルラベル、セッション識別子を変えないため、Pi など他のインテグレーションとの互換性も保たれます。

タイトルは 1–7 語、表示上 48 文字以内です。エンジンはターミナルエスケープ、制御文字、パス、ユーザー識別子、secret のような値を除去し、最後の意味のある段落から具体的な成果を導きます。短い継続指示やメタ指示だけの場合、そのペイン、プロバイダー、ネイティブセッションの最初のブリーフィング題名を維持します。変更のないタイトルはスキップされ、受理した更新は報告元のペイン、プロバイダー、ネイティブセッション ID、単調増加するシーケンスに結び付けられます。古いターンやセッションから遅れて届いた報告は、新しいタイトルを置き換えられません。フック、ソケット、ペイン、独立サブエージェントペインが未対応の場合は何もせず終了します。
