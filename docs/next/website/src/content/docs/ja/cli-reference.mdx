...
`theme status` はホスト値、ランタイムオーバーライド、有効な外観、選択中のテーマを報告します。`theme set` はサーバーが管理するオーバーライドを即座に変更し、購読中の子ペインへ外観の変更を送信します。ランタイム値は次回の設定リロードまたはサーバー再起動まで有効です。永続的にオーバーライドするには `[theme] host_appearance` を設定してください。
...
グローバルナビゲーションでは、ウィンドウはタブを表します。これらのコマンドは、エージェントのないタブも含め、すべてのワークスペースのタブを正規のワークスペース・タブ順で前後にフォーカスし、両端でラップします。
...
`idle` は入力待ちで、そのペインがフォーカス中の Herdr UI ですでに開かれた状態です。`done` は同じ基礎的な idle 状態ですが、表示していないバックグラウンド作業が完了したときに使われます。そのエージェントのペインを開くか `pane focus` / `agent focus` で対象にすると表示済みになりますが、同じタブ内の別のペインを開いたり、CLI で読み取ったりするだけでは変わりません。`blocked` は承認または質問 UI を Herdr が認識した状態です。`unknown` はエージェントが存在するものの確実に分類できない状態で、作業の成功を意味しません。
...
フリートステータスは設定済みのすべてのホストを並列に読み取り、ペインを持つ Herdr エージェントとウィンドウレスな `~/.agents/runs/<run_id>/state.json` レコードを統合します。JSON の `source` フィールドは常に `herdr` または `run_state` のいずれかなので、利用側が証拠の種類を取り違えることはありません。run-state ファイルが存在しないのは正常です。`schema` が `1` 以外の run-state ファイルは、目立つ `run_state` エラー行として表示されます。

人間向けのテーブルはブロック中の行を先頭に、次に作業中またはアクティブな行を並べます。`--blocked-only` は生存状態とは独立にブロック事実だけで絞り込みます。`--watch` は初回スナップショットの後にのみ JSON Lines を出力し、観測した状態遷移をそれぞれ一度だけ出力します。新たにブロックされたイベントにはゲートの要約が含まれます。

フリートステータスは 2 つの独立した事実をモデル化します:

| 入力の証拠 | 生存状態 | ブロック |
| --- | --- | --- |
| Herdr の `idle`、`working`、`blocked` | live | `blocked` または未回答のゲートがあれば true |
| Herdr の `done` | そのエージェントに限り terminal | 未回答のゲートが残っていれば true |
| Herdr の `stale` または `unknown` | unknown | 未回答のゲートが残っていれば true |
| 新しいハートビートを持つ実行の `active`、`waiting`、`blocked` | live | `blocked` の場合のみ true |
| ハートビートが古い、または無効な非終了の実行 | unknown | 実行のブロック事実を保持 |
| 実行の `done` または `failed` | terminal | false |
| ホストの読み取り失敗、または消えた行 | unknown | 保持された証拠がブロックを示さない限り false |

この分離により、静かなゲートが stale に格下げされることを防ぎます。未回答のゲートを持つ古い Herdr レコードは `BLOCKED · UNKNOWN` として表示されます。また、証拠なしに沈黙を完了やブロックと呼ぶことも防ぎます。

ウィンドウレスな実行レコードは、親の実行やネイティブのエージェントセッションを指定できます。フリートステータスは `closure_liveness` と `closure_blocked` を計算するとき、既知のすべての子孫を評価します。`done` を報告する親は自分の行に対してのみ terminal であり、live、unknown、またはブロック中の子があれば親のクロージャは非終了またはブロックのままです。run-state コントラクトにない関係は推測されません。
