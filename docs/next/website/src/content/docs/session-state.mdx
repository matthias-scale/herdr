...
### Continuing a resumed agent

Resuming replays the conversation; it does not restart the work. Once a resumed
agent comes back up at its own idle prompt, Herdr submits one short prompt so it
picks the work back up. This is on by default:
...
Herdr sends the prompt at most once per resume, and only when the agent is
genuinely idle and ready. It stays quiet when the agent comes back blocked on a
question, when the agent resumes straight back into work on its own, and when
the pane already holds text you typed. If the agent never reaches a ready prompt
within two minutes, Herdr drops the nudge rather than sending it late.

You can also toggle it from Settings, under **Continue resumed agents**.
