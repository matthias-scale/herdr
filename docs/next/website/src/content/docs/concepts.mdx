...
The sidebar is one Spaces tree. Every workspace keeps its rolled-up status row and can independently disclose its tabs/windows underneath, including tabs without a detected agent. Each tab is exactly one status-first title row; multiple panes roll up into that owning tab instead of adding model, agent, or pane subtitle rows. Selected Space and tab titles use darker bold text, and agent-backed tab rows end with a compact age for the latest user/agent activity across their panes. Linked-worktree disclosure remains separate.
