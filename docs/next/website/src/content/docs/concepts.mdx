...
The default sidebar is one Spaces tree. Every workspace keeps its rolled-up status row and can independently disclose its tabs and panes underneath, including tabs and panes without a detected agent. Linked-worktree disclosure and pane disclosure are separate controls.
