...
The sidebar is one Spaces tree. Every workspace keeps its rolled-up status row and can independently disclose its tabs/windows underneath, including tabs without a detected agent. Each tab is exactly one status-first title row; multiple panes roll up into that owning tab instead of adding model, agent, or pane subtitle rows. Linked-worktree disclosure remains separate.
