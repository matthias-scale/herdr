...
```bash
herdr theme status [--json]
herdr theme set light|dark|auto
```

`theme status` reports the host value, runtime override, effective appearance, and selected theme. `theme set` changes the server-owned override immediately and sends an appearance transition to subscribed child panes. The runtime value lasts until the next config reload or server restart. Set `[theme] host_appearance` for a durable override.
...
herdr workspace create [--cwd PATH] [--label TEXT] [--env KEY=VALUE] [--ticket ID]... [--pr URL --branch BRANCH --role ROLE [--active-owner]] [--focus] [--no-focus]
herdr workspace get <workspace_id>
herdr workspace focus <workspace_id>
herdr workspace rename <workspace_id> <label>
herdr workspace bind <workspace_id> <owner/repo> [--reconcile]
...
Creation commands can bind the new pane to one pull request before its shell starts. A PR binding requires one `--pr` and a `--role` of `baseline`, `repair`, `ship`, or `review`; `--active-owner` asks Herdr to make that pane the active owner. If the PR already has a live owner, Herdr keeps the existing owner and returns the new pane with `active_owner: false`, so the run remains attached as history. When the owner pane exits, surviving runs show the PR as un-owned.

## Windows

```bash
herdr window next
herdr window previous
```

A window is a tab for global navigation. These commands focus the next or previous tab across every workspace in canonical workspace/tab order, include tabs without agents, and wrap at both ends.
...
herdr pane work-context get <pane_id>
herdr pane work-context set <pane_id> [--ticket ID]... [--pr URL]... [--branch BRANCH] [--repo OWNER/REPO] [--title TITLE] [--clear FIELD]...
herdr pane input [<pane_id>|--pane ID|--current] --right-click herdr|pane
herdr pane split [<pane_id>|--pane ID|--current] --direction left|right|up|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--right-click herdr|pane] [--ticket ID]... [--pr URL --branch BRANCH --role ROLE [--active-owner]] [--focus] [--no-focus]
herdr pane swap --direction left|right|up|down [--pane ID|--current]
herdr pane swap --source-pane ID --target-pane ID
herdr pane move <pane_id> --tab <tab_id> --split left|right|up|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]
...
`pane work-context set` applies one atomic manual patch. Each of `--ticket` and
`--pr` holds one value, and repeating either flag keeps its last value; omitted
fields stay unchanged. `--clear` accepts `ticket_ids`, `pr_urls`, `branch`, `repo`, or `work_title`.
`pane work-context get` returns the same `PaneInfo` response as `pane get`,
including its effective `work_context` object. In the TUI, right-clicking a
pull-request or ticket URL printed in a terminal pane offers **Link PR to this
window** or **Link ticket to this window**, which applies the same manual
`--pr` or `--ticket` patch to every pane of that window and confirms it with a
toast. Once every pane declares that link, the same click offers **Unlink PR
from this window**, which clears the manual binding again. A link only the hook
or git observation tiers detected offers neither, since a declaration cannot
remove an observation.

`workspace bind` records the repository a workspace collects work for, so the
grouping survives new panes instead of having to be rebuilt by hand. A pane is
placed in the bound workspace once it resolves its own repository, which comes
either from a declaration (`pane work-context set --repo`, or a `--pr` spawn
binding, which implies its repository) or, failing that, from the `origin`
remote of the pane's checkout. Declarations outrank the checkout, because a
session often runs from a shared worktree while working on another repository.
Repositories are GitHub-only: a remote on any other host is refused rather than
reduced to its `owner/repo` path, which would otherwise route a mirror into the
wrong workspace.

Placement never takes focus, and the pane the human is currently focused in is
never moved; it is placed later, once focus has moved on. `--reconcile` sorts
panes that already resolved the repository, under the same two rules. A
repository may be bound to only one workspace.

Read output:

```bash
herdr pane read <pane_id> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi] [--raw] [--json]
...
`--json` returns the complete response envelope. An unbounded text read from the
`detection` source includes `terminal_id`, `agent_ref`, `agent_session`, and an
`input_observation.token` when the snapshot is coherent and no input is pending.

Send input:

```bash
herdr pane send-text <pane_id> <text>
herdr pane send-text-if <pane_id> <text> --workspace <workspace_id> --terminal <terminal_id> --agent-ref <host::pane> --agent-source <source> --agent <label> --session-kind <id|path> --session <value> --condition detection-snapshot-unchanged --observation-token <token>
...
`pane send-text-if` is for automation that must not overwrite intervening input.
Copy every identity field and the observation token from the JSON detection read.
It emits `pane_text_send` with `outcome: sent` only after one enqueue; a stale
identity, changed snapshot, or pending input emits `outcome: condition_mismatch`
and sends no bytes. Servers without this method reject it without falling back to
unguarded `pane.send_text`.
...
`report-agent-session` updates native session identity without reporting lifecycle state. `release-agent` ends that source's lifecycle authority when its agent process exits. Use `herdr agent wait <target> --until stale` to wait for the server-side watchdog status; `stale` is never a valid value for `pane report-agent --state`.
...
herdr agent state <pane> --json
herdr agent report <pane> --json [payload]
```
...
`idle` means ready for input after its pane has been opened in the focused Herdr UI; `done` is the same underlying idle state after unseen background work completes. Opening that agent's pane or targeting it with `pane focus` / `agent focus` marks it seen; opening another pane in the same tab or reading it through the CLI does not. `blocked` means Herdr recognized an approval or question UI. `unknown` means an agent is present but Herdr cannot classify it confidently, not that its work succeeded.
...
`agent state` prints the server-owned state Herdr keeps for a pane: the links seen in its output, the agent's status and when it last acted, and any goal, tasks, or subagents the agent reported. The agent tab in the sidebar renders the same state, so this is the way to read it from a script.

`agent report` is how an agent writes that state. The payload is JSON, either as the value of `--json` or on standard input, and it replaces the agent-owned fields only. Links Herdr observed in pane output are not writable this way.
...
```bash
herdr fleet status
herdr fleet status --hosts laptop,workbox --blocked-only
herdr fleet status --json
herdr fleet status --watch
```

Fleet status reads every configured host in parallel and merges pane-backed Herdr agents with windowless `~/.agents/runs/<run_id>/state.json` records. The `source` field remains `herdr` or `run_state` in JSON so consumers never confuse the evidence classes. Missing run-state files are normal. A run-state file with `schema` other than `1` produces a loud `run_state` error row.

The human table sorts blocked rows first, then working or active rows. `--blocked-only` uses the blocked fact, independent of liveness. `--watch` prints JSON Lines only after the initial snapshot and emits each observed state transition once. Newly blocked events include the gate summary.

Fleet status models two separate facts:

| Input evidence | Liveness | Blocked |
| --- | --- | --- |
| Herdr `idle`, `working`, or `blocked` | live | true for `blocked` or any unanswered gate |
| Herdr `done` | terminal for that agent only | true if an unanswered gate remains |
| Herdr `stale` or `unknown` | unknown | true if an unanswered gate remains |
| Run `active`, `waiting`, or `blocked` with a fresh heartbeat | live | true only for `blocked` |
| Non-terminal run with an old or invalid heartbeat | unknown | preserves the run's blocked fact |
| Run `done` or `failed` | terminal | false |
| Host read failure or disappeared row | unknown | false unless retained evidence says blocked |

This split prevents a quiet gate from being demoted to stale. A stale Herdr record with unanswered gates appears as `BLOCKED · UNKNOWN`. It also prevents silence from being called complete or blocked without evidence.

Windowless run records may name a parent run or native agent session. Fleet status scores every known descendant when it computes `closure_liveness` and `closure_blocked`. A parent reporting `done` is terminal only for its own row; a live, unknown, or blocked child keeps the parent closure non-terminal or blocked. Relationships absent from the run-state contract cannot be inferred.
...
```bash
herdr plugin pane open --plugin ID --entrypoint ID [--placement overlay|popup|split|tab|zoomed] [--width SIZE] [--height SIZE] [--workspace ID] [--target-pane PANE] [--direction left|right|up|down] [--cwd PATH] [--env KEY=VALUE] [--focus|--no-focus]
