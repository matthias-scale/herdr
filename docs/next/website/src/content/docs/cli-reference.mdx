...
`pane work-context set` applies one atomic manual patch. Repeated `--ticket` and
`--pr` flags replace those lists in first-seen order; omitted fields stay
unchanged. `--clear` accepts `ticket_ids`, `pr_urls`, `branch`, `repo`, or `work_title`.
