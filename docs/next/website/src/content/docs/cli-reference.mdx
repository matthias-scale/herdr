...
herdr pane split [<pane_id>|--pane ID|--current] --direction left|right|up|down [--ratio FLOAT] [--cwd PATH] [--env KEY=VALUE] [--ticket ID]... [--pr URL --branch BRANCH --role ROLE [--active-owner]] [--focus] [--no-focus]
