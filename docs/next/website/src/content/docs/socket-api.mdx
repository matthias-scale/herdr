...
| Theme | `theme.status`, `theme.set` |
| Notification | `notification.show` |
| Client | `client.window_title.set`, `client.window_title.clear` |
| Session | `session.snapshot` |
| Group | `group.host_snapshot`, `group.create`, `group.rename`, `group.delete` |
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.bind`, `workspace.move`, `workspace.move_block`, `workspace.report_metadata`, `workspace.close` |
| Loop | `loop.list`, `loop.run_history` |
...
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.group.set`, `pane.work_context.set`, `pane.send_text`, `pane.send_text_if`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
| Layout | `layout.export`, `layout.apply`, `layout.set_split_ratio` |
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.state`, `agent.report`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.focus.status`, `agent.start`, `agent.view.set`, `agent.view.clear` |
| Fleet | `fleet.list` |
...
`loop.run_history` is read-only. Omit `loop_id` to return every observed receipt-backed run, or pass one loop id to filter records when the receipts carry loop identity. During the receipt-schema migration, a history with no loop identities remains unfiltered. The response contains the selected `loop_id`, typed `runs`, and `skipped_lines` for malformed receipt rows. Reading it does not open or change the TUI detail view.

```json
{"id":"history","method":"loop.run_history","params":{"loop_id":"daily"}}
```

`fleet.list` returns the server-owned, most recently polled fleet snapshot. It is
read-only and never waits for SSH. The response distinguishes an inventory that
has never been polled from a completed poll with no running agents, and includes
each configured host's reachability, version, protocol, error, and agents.
Every fleet agent has an `agent_ref` string in `host::agent` form. `agent.list`
returns the same field for local agents, using `remote.fleet.self_name` as its
host component.

`agent.focus` keeps the existing `target` form for local focus and also accepts
an `agent_ref`. A self-host reference follows the synchronous local path. A
configured remote host returns an operation in `connecting` state; poll it with
`agent.focus.status` using the returned `operation_id`. Remote focus uses the
configured host name in the API and UI, even when the remote server reports a
different name for itself. Focus is unavailable with `host_unreachable` until a
successful fleet poll has learned one unambiguous identity for that configured
host; the poll itself remains read-only and does not wait for focus.

```json
{"id":"fleet","method":"fleet.list","params":{}}
```
...
`group.host_snapshot` returns the active and deleted group records owned by
this server, plus its current pane memberships. Group mutations use optimistic
revisions: `group.create` compares the store revision, while `group.rename`,
and `group.delete` compare the group record revision. `pane.group.set` compares
the pane membership revision. A successful mutation is durable before the
response is sent. Group ids contain the durable server authority, so names and
configured fleet aliases are not identity.
...
`PaneInfo` and `AgentInfo` include the pane-owned effective `work_context`:

```json
{
  "ticket_ids": ["MAT-123"],
  "pr_urls": ["https://github.com/owner/repo/pull/42"],
  "preview_urls": ["https://herdr-preview.vercel.app"],
  "branch": "feat/work-context",
  "work_title": "Add pane work context"
}
```

Set or clear manual context fields atomically with `pane.work_context.set`.
Omitted fields remain unchanged. A field cannot appear in both its setter and
`clear_fields`. Ticket IDs are canonicalized to uppercase and pull request URLs
must be HTTPS GitHub `/pull/<positive-number>` URLs.

```json
{
  "id": "req_context",
  "method": "pane.work_context.set",
  "params": {
    "pane_id": "w1:p1",
    "ticket_ids": ["mat-123"],
    "pr_urls": ["https://github.com/owner/repo/pull/42"],
...
Preview URLs are derived only from guarded agent turn hooks and must be HTTPS
root URLs for a single `<subdomain>.vercel.app` host. Manual context updates and
Git observations do not create preview URLs. Valid fields for `clear_fields`
are `ticket_ids`, `pr_urls`, `branch`, and `work_title`. Successful mutations
emit `pane.updated` with the new effective context.
...
`direction` (`left`, `right`, `up`, or `down`), `ratio`, `first`, and `second`.
...
`agent.view.set` installs one transient declarative compatibility projection.
The projection is reevaluated whenever agent facts or current UI context
change and controls indexed Agent focus order. The visible desktop, collapsed,
and mobile sidebar remains the canonical Spaces → tab/window tree; panes roll
up into their owning tab's single row;
the projection does not filter or reorder its rows or mouse targets.
Configured next/previous Agent actions likewise use the canonical unfiltered
workspace, tab, and pane order. The projection does not change `agent.list`,
notifications, detection, or global attention counts.

Define an indexed-focus projection containing agents in the currently
presented Space or agents needing attention elsewhere, ordered by attention
and most recent state transition:
...
```json
{
  ...
  "params": {
    ...
    "wait": "CI run 4123",
    "eta_s": 720,
    "reported_at": "2026-08-10T10:00:00Z"
  }
}
```

`state` is semantic. It affects waits, notifications, and rollups. A working
turn may declare a wait and its deadline with `wait` and `eta_s`. Every accepted
report may carry `reported_at`, which Herdr exposes back through pane/agent
responses and status events. `stale` is supervisor-derived after a missed
deadline or prolonged silence; integrations must never report it. Report
...
Loop history subscriptions use `loop.run_history_updated`. Herdr emits it when the receipt-backed history changes. The event carries `loop_id`, the current typed `runs`, and `skipped_lines`, matching the read-only history response shape.
...
`pane.agent_status_changed` includes `wait`, `eta_s`, and `reported_at` when
the report supplied them. Its `agent_status` may be `stale` when the
server-side watchdog finds a reporting violation.
...
herdr pane read w1:p1 --source detection --json
```
...
The JSON form of an unbounded text detection read may include an
`input_observation.token` plus exact pane, workspace, terminal, agent reference,
and agent session identity. Send those unchanged to `pane.send_text_if` with the
`detection_snapshot_unchanged` condition. The response is `pane_text_send` with
`outcome` set to `sent` or `condition_mismatch`; mismatch sends zero bytes.
...
herdr agent wait w1:p1 --until stale
```
...
`theme.status` reads the server-owned appearance state. `theme.set` changes the runtime override to `light`, `dark`, or `auto` and returns the updated status:

```json
{"id":"theme_1","method":"theme.set","params":{"host_appearance":"light"}}
```
