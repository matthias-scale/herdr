...
context change. It replaces the Spaces tree with one flat projection in the
expanded and collapsed sidebar and mobile navigation, and controls its mouse
targets and indexed focus. Configured next/previous Agent actions continue to
use the canonical unfiltered workspace, tab, and pane order. The projection
