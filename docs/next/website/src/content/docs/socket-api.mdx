...
`agent.focus.status` using the returned `operation_id`. Remote transport is not
available until the remote-control protocol slice lands, so the current stub
settles those operations as `failed` with `host_unreachable`.
