...
## Automatic work titles

Claude Code and Codex integrations recalculate the owning tab's single Herdr title on every `UserPromptSubmit`, before the provider begins the turn. Automatic work titles require **integration version 8** for both Claude Code and Codex; run `herdr integration status` and reinstall an integration reported as outdated. Each hook invokes the Herdr binary that owns its pane, and both adapters call the same deterministic title engine; there is no extra model request or prompt context. Pi and other integrations remain compatible because work titles use presentation metadata without changing lifecycle authority, native model labels, or session identifiers.

Titles contain 1–7 words and at most 48 visible characters. The engine removes terminal escapes, control characters, paths, user identifiers, and secret-like values, then derives a concrete outcome from the latest meaningful final paragraph. A terse or meta-only continuation retains that pane, provider, and native session's initial briefing subject. Herdr skips an unchanged title and binds every accepted update to the reporting pane, provider, native session ID, and monotonic sequence. A delayed report from an older turn or session cannot replace a newer title. Missing hook, socket, pane, or independent subagent pane support is a silent no-op.
