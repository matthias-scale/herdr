...
Claude Code and Codex integrations recalculate the pane's Herdr agent title on every `UserPromptSubmit`, before the provider begins the turn. Automatic work titles require **integration version 8** for both Claude Code and Codex; run `herdr integration status` and reinstall an integration reported as outdated. Both adapters call the same deterministic title engine; there is no extra model request or prompt context. Pi and other integrations remain compatible because work titles use presentation metadata without changing lifecycle authority, native model labels, or session identifiers.
