...
Titles contain 1–7 useful words and at most 48 visible characters. The engine removes terminal escapes, control characters, paths, user identifiers, and secret-like values, then derives a concrete outcome phrase from the latest objective. A terse continuation can retain the existing normalized title. Herdr skips an unchanged title and binds every accepted update to the reporting pane, provider, native session ID, and monotonic sequence. A delayed report from an older turn or session cannot replace a newer title. Missing hook, socket, pane, or independent subagent pane support is a silent no-op.
