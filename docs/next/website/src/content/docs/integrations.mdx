...
Claude Code and Codex integrations recalculate the pane's Herdr agent title on every `UserPromptSubmit`, before the provider begins the turn. Both adapters call the same deterministic title engine; there is no extra model request or prompt context. Pi and other integrations remain compatible because work titles use presentation metadata without changing lifecycle authority, native model labels, or session identifiers.
