...
如果在 `agent_detection.full_lifecycle_hook_authority_timeout_seconds` 内没有新的已接受上报，完整生命周期权威就会过期。默认值为 600 秒，有效范围为 30–3600 秒。与上报矛盾的窗格输出也可能提前撤销 blocked 钩子权威。权威撤销后，当前屏幕结果决定窗格状态；之后到达的有效生命周期上报可以重新取得权威。回合结束时的 `herdr:<agent>-closing-block` 上报可以携带有效门控，但不会取得完整生命周期权威，也不会压制更新的屏幕证据。
