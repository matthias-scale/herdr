...
父智能体结束回合时若声明仍有子智能体在运行，会显示为带黄色空心点的 `waiting on agents`，而不是 done 或普通 working。检测缓冲区的输出发生变化时会重置静默计时，仅有输入不会重置。默认的 30 分钟预算可通过 `session.agent_subagent_stale_after_minutes` 配置；后续声明运行中智能体为零的 closing report 会结束等待。即使仍有子进程，已完成的 closing report 仍显示 done。
