...
对于具备完整生命周期钩子的智能体，最近接受的集成上报在常规 `idle`、`working` 和 `blocked` 仲裁中具有权威。Herdr 仍会同时评估屏幕清单。原生权限、信任、登录、工具审批或提问控件不一定发出生命周期事件，因此，如果这些结构明确的控件比非 blocked 的钩子上报更新，它们可以覆盖该上报。对于普通提示框或 working 屏幕证据，新鲜钩子状态仍然优先。

如果在 `agent_detection.full_lifecycle_hook_authority_timeout_seconds` 内没有新的已接受上报，完整生命周期权威就会过期。默认值为 600 秒，有效范围为 30–3600 秒。与上报矛盾的窗格输出也可能提前撤销 blocked 的完整生命周期权威。权威撤销后，当前屏幕结果决定窗格状态；之后到达的有效生命周期上报可以重新取得权威。回合结束时的 `herdr:<agent>-closing-block` 上报可以携带有效门控而不取得完整生命周期权威。`blocked` closing gate 在终端持续输出时仍保持权威；明确输入、清除门控的新上报、智能体替换或进程退出会撤销它。

父智能体结束回合时若声明仍有子智能体在运行，会显示为带黄色空心点的 `waiting on agents`，而不是 done 或普通 working。检测缓冲区的输出发生变化时会重置静默计时，仅有输入不会重置。默认的 60 分钟预算可通过 `session.agent_subagent_stale_after_minutes` 配置；后续声明运行中智能体为零的 closing report 会结束等待。即使仍有子进程，已完成的 closing report 仍显示 done。
...
在 Claude Code CLI 内运行的 Kimi 通过 Claude 进程和 Claude 屏幕清单检测。它不依赖 Kimi 钩子、fleet closing-block token，也不依赖特定的模型横幅或状态行。
...
Claude 和 Codex 的套餐用量耗尽屏幕属于另一类 blocker。人无法通过回答解除它，因此侧边栏将其标为 `usage`。该状态由屏幕实时决定，不会锁存；屏幕显示恢复工作后会立即清除。
...
实时 explain 由运行中的服务器评估，因此反映的是生效的清单缓存。它会分别显示清单推导出的 `screen_state` 和最终 `state`，并给出胜出的 `arbitration`，例如 `fresh_full_lifecycle_hook`、`visible_blocker_over_hook` 或 `screen`。即使钩子权威仍新鲜，Herdr 也继续评估屏幕规则，所以钩子胜出时仍能查看匹配器和区域证据。输出还包含清单来源和版本、缓存的远程版本、本地覆盖的遮蔽情况、远程更新状态、匹配的规则、可见证据标志、转写查看器的跳过更新原因，以及没有规则匹配时的 idle 回退原因。
...
一个 blocked 的智能体会让它的窗格、标签页和工作区看起来是 blocked。等待子智能体的父智能体显示为带黄色空心点的 `waiting on agents`。一个 working 的智能体会让工作区看起来处于活跃状态。一个 done 的智能体在你查看之前会一直保持可见。working 长时间无报告，或等待子智能体超过配置的静默预算时，会变为 `stale`。
