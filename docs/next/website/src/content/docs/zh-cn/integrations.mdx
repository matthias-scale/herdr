...
Claude Code 和 Codex 集成会在每次 `UserPromptSubmit` 时、提供方开始该轮之前重新计算所属标签页唯一的 Herdr 标题。自动工作标题要求 Claude Code 和 Codex 都使用**集成版本 8**;请运行 `herdr integration status`,并重新安装报告为过期的集成。每个钩子都会调用拥有其窗格的 Herdr 二进制文件;两个适配器共用同一个确定性标题引擎,不会增加模型调用或提示词上下文。标题包含 1–7 个词,最多 48 个可见字符。引擎会移除控制字符、终端转义、路径、用户标识符和疑似机密值,再从靠近末尾的最后一个有意义段落中提取具体目标。若后续提交只有简短的继续指令或元指令,标题会保留该窗格、提供方和原生会话各自最初简报的主题。

每次更新都绑定到窗格、提供方、原生会话 ID 和单调递增序列。相同的规范化标题不会重复写入，旧轮次或旧会话的延迟更新无法覆盖新标题。缺少所需钩子、套接字、窗格或子代理独立窗格时，会安全地静默跳过。
...
该钩子会在 Grok CLI 运行于 Herdr 窗格内时，通过 Grok 的 `SessionStart` 钩子上报会话身份。Grok 的状态来自 Herdr 的屏幕清单检测。

Herdr 默认使用 `~/.grok`，设置了 `GROK_HOME` 时使用后者。Grok 配置目录必须已经存在。Grok 会合并该目录中的所有 `hooks/*.json` 文件，因此安装会在 `hooks/herdr-agent-state.sh` 脚本旁写入一个自包含的 `hooks/herdr.json`，其中包含 Herdr 的 `SessionStart` 条目，并且不会编辑其他钩子文件。卸载只会删除这两个由 Herdr 管理的文件。

Grok 发出会话启动事件后，Herdr 可以使用上报的会话 id，通过 `grok --resume <id>` 恢复该窗格。
