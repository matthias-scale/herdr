...
Claude Code 和 Codex 集成会在每次 `UserPromptSubmit` 时、提供方开始该轮之前重新计算所属标签页唯一的 Herdr 标题。自动工作标题要求 Claude Code 和 Codex 都使用**集成版本 8**;请运行 `herdr integration status`,并重新安装报告为过期的集成。每个钩子都会调用拥有其窗格的 Herdr 二进制文件;两个适配器共用同一个确定性标题引擎,不会增加模型调用或提示词上下文。标题包含 1–7 个有效词,最多 48 个可见字符,并移除控制字符、终端转义、路径、用户标识符和疑似机密值。
