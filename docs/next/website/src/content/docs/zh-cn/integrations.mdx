...
Claude Code 和 Codex 集成会在每次 `UserPromptSubmit` 时、提供方开始该轮之前重新计算 Herdr 代理标题。两个适配器共用同一个确定性标题引擎，不会增加模型调用或提示词上下文。标题包含 3–7 个词，最多 48 个可见字符，并移除控制字符、终端转义、路径、用户标识符和疑似机密值。
