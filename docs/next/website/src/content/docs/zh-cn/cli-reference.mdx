...
`theme status` 报告主机值、运行时覆盖值、当前生效的外观和所选主题。`theme set` 立即更改服务器管理的覆盖值,并向已订阅的子窗格发送外观变更。该运行时值会持续到下一次重载配置或重启服务器。若要持久覆盖,请设置 `[theme] host_appearance`。
...
在全局导航中,窗口就是标签页。这些命令按规范的工作区、标签页顺序聚焦所有工作区中的下一个或上一个标签页,包括没有智能体的标签页,并在两端循环。
...
`idle` 表示智能体在等待输入,且其窗格已在聚焦的 Herdr 界面中打开。`done` 是相同的底层 idle 状态,用于未查看的后台工作完成后。打开该智能体的窗格或用 `pane focus` / `agent focus` 指向它会标记为已查看;在同一标签页中打开其他窗格或仅通过 CLI 读取都不会改变它。`blocked` 表示 Herdr 识别到审批或提问界面。`unknown` 表示智能体存在但无法可靠分类,不代表工作成功。
...
集群状态会并行读取每个已配置的主机，并把有窗格支撑的 Herdr 代理与无窗口的 `~/.agents/runs/<run_id>/state.json` 记录合并在一起。JSON 中的 `source` 字段始终为 `herdr` 或 `run_state`，因此使用方不会混淆两类证据。run-state 文件缺失是正常情况。`schema` 不为 `1` 的 run-state 文件会产生一条醒目的 `run_state` 错误行。

人类可读的表格先列出被阻塞的行，然后是工作中或活跃的行。`--blocked-only` 只依据阻塞事实筛选，与存活状态无关。`--watch` 仅在初始快照之后输出 JSON Lines，并且每个观察到的状态转换只输出一次。新出现的阻塞事件包含闸门摘要。

集群状态把两个独立的事实分开建模：

| 输入证据 | 存活状态 | 阻塞 |
| --- | --- | --- |
| Herdr 的 `idle`、`working` 或 `blocked` | live | `blocked` 或任何未回答的闸门为 true |
| Herdr 的 `done` | 仅对该代理为 terminal | 仍有未回答的闸门时为 true |
| Herdr 的 `stale` 或 `unknown` | unknown | 仍有未回答的闸门时为 true |
| 带有新鲜心跳的运行 `active`、`waiting` 或 `blocked` | live | 仅 `blocked` 为 true |
| 心跳过期或无效的未结束运行 | unknown | 保留该运行的阻塞事实 |
| 运行 `done` 或 `failed` | terminal | false |
| 主机读取失败或行已消失 | unknown | 除非保留的证据表明阻塞，否则为 false |

这种拆分可以防止一个安静的闸门被降级为 stale。带有未回答闸门的过期 Herdr 记录会显示为 `BLOCKED · UNKNOWN`。它同样防止在没有证据的情况下把沉默当作完成或阻塞。

无窗口的运行记录可以指定父运行或原生代理会话。集群状态在计算 `closure_liveness` 和 `closure_blocked` 时会评估所有已知的后代。报告 `done` 的父运行只对它自己的行是 terminal；一个 live、unknown 或阻塞中的子运行会让父闭包保持未结束或阻塞。run-state 契约中不存在的关系不会被推断。
