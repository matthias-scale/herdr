...
添加 `[[remote.fleet.hosts]]` 条目后，这些主机会出现在 Hosts 停靠面板中。
herdr 会在后台刷新它们的可达性、版本和代理清单。选择一个可达的远程主机，
即可在新的本地窗格中打开 `herdr --remote`。
...
可选的 `session` 值会在打开主机时附加 `--session`。标记为 `local = true` 的
主机使用本地 API 套接字读取清单，不会通过 SSH 轮询。当 Hosts 面板为空时，
它会指引你配置 `[[remote.fleet.hosts]]`。
