...
`loop.run_history` 是只读方法。省略 `loop_id` 会返回回执中观察到的所有运行记录；传入一个循环 id 时，如果回执包含循环标识，则只返回带该标识的记录。在回执 schema 迁移期间，如果历史中的记录都没有循环标识，结果不会过滤。响应包含所选 `loop_id`、类型化的 `runs`，以及格式错误的回执行数 `skipped_lines`。读取历史不会打开或修改 TUI 详情视图。
...
响应包含 `workspace_id`、`tab_id`、`zoomed`、`focused_pane_id` 和 `root`。`root` 是由 `pane` 和 `split` 节点组成的 BSP 树。窗格节点可以包含 `pane_id`、`label`、`cwd` 和 argv `command`。分割节点使用 `direction` (`left`、`right`、`up` 或 `down`)、`ratio`、`first` 和 `second`。
...
`agent.view.set` 设置一个临时的声明式兼容投影。智能体信息或当前 UI 上下文变化时,
该投影会重新求值,并控制编号 Agent 聚焦顺序。可见的桌面、折叠和移动侧边栏仍是
规范的 Spaces → 标签页 → 窗格归属树;该投影不会过滤或重排其中的行或鼠标目标。
配置的上一个/下一个智能体操作同样使用未过滤的工作区、标签页和窗格规范顺序。
该投影不会改变 `agent.list`、通知、检测或全局待处理计数。

下面的示例定义一个编号聚焦投影,其中包含当前呈现的 Space 中的智能体以及其他位置
需要处理的智能体,并按待处理优先级和最近状态转换排序:
...
循环历史订阅使用 `loop.run_history_updated`。回执记录的历史发生变化时，Herdr 会发出该事件。事件包含 `loop_id`、当前的类型化 `runs` 和 `skipped_lines`，与只读历史响应的结构一致。
