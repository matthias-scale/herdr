...
恢复只会重放对话,并不会重启工作。当恢复后的智能体回到自己的空闲提示符时,Herdr 会发送一条简短提示,让它把手上的活接着做下去。该功能默认开启:
...
每次恢复最多发送一次,并且只在智能体确实空闲、可以接收输入时发送。如果智能体恢复后正卡在一个待回答的问题上、恢复后自己已经继续工作,或者窗格里还留着你输入的草稿,Herdr 都不会发送。如果两分钟内智能体仍未到达可接收输入的提示符,Herdr 会放弃发送,而不是延后补发。

也可以在设置中的 **Continue resumed agents** 一项里切换。
