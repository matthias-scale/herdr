...
桌面布局会在标签栏上方显示一条全宽状态行，其中包含当前聚焦会话的标识、目录和 Git 分支，可用的网络和电池信息，以及日期和时间。在桌面宽度下，带有 `MEM` 和 `CPU` 字样的片段始终可见，分别使用 GiB 和百分比作为单位；样本不可用时会显示 `MEM --/-- GiB` 或 `CPU --%`。随着终端变窄，可选片段会按以下顺序消失：时间、日期、电池、IP 地址、带宽、Git 分支、目录、用户、主机。
