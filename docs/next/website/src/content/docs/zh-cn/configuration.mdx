...
侧边栏是 Herdr 的主仪表盘。默认的 `spaces` 排序会把它渲染成一棵工作区树：每个工作区都保留自己的汇总行，并可以在其下方展开检测到的智能体。关联 worktree 的展开和智能体的展开互相独立。紧凑视图和移动视图保留同一棵归属树。

设置 `ui.agent_panel_sort = "priority"` 会把这棵树替换成一个按待处理优先级排序的扁平 Agent 投影。临时的 `agent.view.set` 查询同样会把它替换成一个带标签的过滤投影。打开工作区选择器只会临时恢复 Spaces 树，不会清除任何一种投影。在[配置参考](/docs/config-reference/)中搜索 `ui.`，可查看尺寸、折叠模式、排序、鼠标行为、窗格边框和其他显示设置。

每个展开的 Agent 窗口行末尾都会显示紧凑的活动时长。工作中的窗口显示当前活动区间的持续时间；空闲窗口显示自最近一次活动区间结束后的时间。这个固定宽度字段只在显示值变化时刷新，因此标签保持对齐；侧边栏变窄时，它会先于窗口上下文被省略。

### 状态行

桌面布局会在标签栏上方显示一条右对齐的全宽状态行。信息顺序是聚焦文件夹、Git 分支、设备名、Herdr 构建版本、CPU、内存；左侧始终留空。桌面宽度下，构建版本以及 `CPU` 和 `MEM` 片段始终可见。指标分别使用百分比和 GiB；样本不可用时显示 `CPU --%` 或 `MEM --/-- GiB`。状态行不显示会话或窗口标识、网络信息、用户名、电池、日期或时间。终端变窄时，可选片段按文件夹、分支、设备名的顺序消失。

采样器会立即启动，每两秒刷新一次，同时最多只允许一个样本正在采集；失败后会在下一个间隔重试。达到四秒的旧样本会被丢弃并显示不可用值。平台指标只来自原生 API 或本地 proc/sys 文件。仅当 Herdr 能安全读取新的兼容 tmux-powerline WAN 缓存时才会显示公网 IP；Herdr 绝不会发起公网 IP 请求。
...
`row_gap` 控制 Space 根条目之间，以及 Agent 子条目或扁平 Agent 条目之间的空白终端行数。Agent 和 Space 的取值仍然互相独立。默认值为 `0`，会紧密排列条目；设为 `1` 可恢复之前的间距。它不会在 `rows` 声明的内容行之间添加间距。连续缩进的 worktree 子项仍会作为一个 Space 组紧密排列。
...
侧边栏行设置仅影响展开的桌面侧边栏。折叠视图和移动视图会以紧凑方式渲染同一棵归属与展开树。
