...
侧边栏是 Herdr 的主仪表盘。默认的 `spaces` 顺序是一棵工作区→标签页→窗格的归属树。所有标签页和窗格都按布局顺序显示，包括未检测到智能体的标签页。关联 worktree 和窗格的展开状态相互独立，紧凑视图和移动视图也保留同一棵树。选中状态通过前景色和粗体显示，不使用强背景色。
...
每个展开的标签页只显示一次已保存的标题，然后在其下显示窗格行。窗格状态始终明确显示为 `working`、`blocked` 或 `done`；打开已完成的窗格会清除 `done`，但不会移动其标签页。活动时长只是补充信息，不会取代尚未查看的 `done` 状态。
...
`row_gap` 控制 Space 根条目之间以及窗格子条目之间的空白终端行数。Agent 和 Space 的取值仍然互相独立。默认值为 `0`，会紧密排列条目；设为 `1` 可恢复之前的间距。它不会在 `rows` 声明的内容行之间添加间距。连续缩进的 worktree 子项仍会作为一个 Space 组紧密排列。
