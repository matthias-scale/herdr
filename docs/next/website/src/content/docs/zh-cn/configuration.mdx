...
`ui.agent_panel_sort = "priority"` 仍会为了兼容性和注意力摘要而被接受，但最终侧边栏始终保持标准的工作区、标签页和窗格顺序。生命周期变化不会重排这些归属行，临时的 `agent.view.set` 查询也会保留归属投影。在[配置参考](/docs/config-reference/)中搜索 `ui.`，可查看尺寸、折叠模式、鼠标行为、窗格边框和其他显示设置。
