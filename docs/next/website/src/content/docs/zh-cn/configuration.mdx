...
当一行只属于一个已识别的提供方时，标题后会追加短标识：Claude 使用 `· cc`，Codex 使用 `· cx`，Pi 使用 `· pi`。没有智能体、不受支持、混合提供方或智能体已退出的行不显示该标识。当 Codex 明确报告后台终端时，非零数量会在该标识后显示为 `N >_`。数量会在标签页拥有的所有窗格间求和，但不会改变窗口生命周期状态或排序。最近通信时长仍保持右对齐。
