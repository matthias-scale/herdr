...
侧边栏是 Herdr 的主仪表盘。默认的 `spaces` 顺序是一棵按仓库组织的归属树：仓库与其关联 worktree 共用一个 Space 行；每个标签页都直接显示为一个窗口子项，包括未检测到智能体或包含多个窗格的标签页。紧凑视图和移动视图也保留同一棵树。选中状态通过前景色和粗体显示，不使用强背景色。

`ui.agent_panel_sort = "priority"` 仍会为了兼容性和旧版编号 Agent 投影而被接受,但可见侧边栏始终保持标准的工作区、标签页和窗格顺序。生命周期变化不会重排这些归属行。临时的 `agent.view.set` 查询也不会改变归属树;其兼容行为见[智能体视图查询](/zh-cn/docs/socket-api/#智能体视图查询)。在[配置参考](/docs/config-reference/)中搜索 `ui.`,可查看尺寸、折叠模式、鼠标行为、窗格边框和其他显示设置。

每个展开的标签页只在一行中显示一次已保存的标题，即使它包含多个窗格。智能体支持的行会将窗格状态汇总为 `working`、`blocked` 或 `done`；没有智能体的行不显示生命周期状态。打开已完成的标签页会清除 `done`，但不会移动它。活动时长只是补充信息，不会取代尚未查看的 `done` 状态。
...
可见侧边栏始终是 Spaces 树。仓库与其关联 worktree 使用固定的单行 Space 条目，其中依次显示展开图标、Space 标题和直接窗口数量。其下每个标签页/窗口也使用固定单行，显示状态、标题和补充信息。
...
为保持配置兼容，`ui.sidebar.spaces.rows` 和 `ui.sidebar.agents` 行声明仍会被接受，但不会改变可见的固定 Space/窗口投影。旧版 Agent 行声明支持以下内置 token：
...
当一行只属于一个已识别的提供方时，标题后会追加短标识：Claude 使用 `· cc`，Codex 使用 `· cx`，Pi 使用 `· pi`。没有智能体、不受支持、混合提供方或智能体已退出的行不显示该标识。当 Codex 明确报告后台终端时，非零数量会在该标识后显示为 `N >_`。数量会在标签页拥有的所有窗格间求和，并在 Codex 退出时清除，但不会改变窗口生命周期状态或排序。最近通信时长仍保持右对齐。在窄行中，Herdr 会依次省略时长、后台终端数量和生命周期文本，然后才截短标题；提供方标识的空间会优先于这些可选字段保留。
