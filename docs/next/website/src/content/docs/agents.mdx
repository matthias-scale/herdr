...
A blocked agent makes its pane, tab, and workspace look blocked. A working agent makes the workspace look active. A done agent stays visible until you view it. If a working agent misses a declared deadline or goes silent without a declared wait, the server derives `stale`; it is shown separately from a plain-shell `unknown` state until a fresh report arrives.
