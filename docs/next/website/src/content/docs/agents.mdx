...
A turn-end report that declares running subagents is shown as `waiting on agents`, with a hollow yellow dot, instead of done or ordinary working. Changed detection-buffer output restarts its quiet clock; input alone does not. The default 30-minute budget is configured with `session.agent_subagent_stale_after_minutes`; a later closing report with zero running agents ends the wait. A finished closing report remains done even if a child process lingers.
