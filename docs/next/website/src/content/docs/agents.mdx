...
For agents with complete lifecycle hooks, a fresh accepted integration report is authoritative for ordinary `idle`, `working`, and `blocked` arbitration. Herdr still evaluates the screen manifest. A newer, structurally anchored native permission, trust, login, tool-approval, or question control can override a non-blocked hook report because those controls may not emit lifecycle events. Fresh hook state still wins over ordinary prompt and working-screen evidence.

Full-lifecycle authority expires when no accepted report arrives within `agent_detection.full_lifecycle_hook_authority_timeout_seconds`. The default is 600 seconds and the valid range is 30–3600. Contradictory pane output can retire blocked full-lifecycle authority earlier. After retirement, the current screen result decides the pane state. A later valid lifecycle report can take authority again. Turn-end `herdr:<agent>-closing-block` reports can carry a live gate without claiming the full lifecycle. A blocked closing gate remains authoritative across terminal output; explicit input, a fresh report that clears the gate, agent replacement, or process exit releases it.

A turn-end report that declares running subagents is shown as `waiting on agents`, with a hollow yellow dot, instead of done or ordinary working. Changed detection-buffer output restarts its quiet clock; input alone does not. The default 60-minute budget is configured with `session.agent_subagent_stale_after_minutes`; a later closing report with zero running agents ends the wait. A finished closing report remains done even if a child process lingers.
...
Kimi running inside the Claude Code CLI is detected through the Claude process and Claude screen manifest. It does not require Kimi hooks, fleet closing-block tokens, or a particular model banner or status line.
...
Claude and Codex plan-exhaustion screens are a separate kind of blocker. The sidebar labels them `usage` because no human answer can release them. This state is screen-backed rather than latched: it clears as soon as the live screen changes to resumed work.
...
Live explain is evaluated by the running server, so it reflects the active manifest cache. It reports the manifest-derived `screen_state` separately from the effective `state` and names the winning `arbitration`, such as `fresh_full_lifecycle_hook`, `visible_blocker_over_hook`, or `screen`. Screen rules remain evaluated while hook authority is fresh, so matcher and region evidence stays available even when a hook wins. The output also includes manifest source and version, cached remote version, local override shadowing, remote update status, matched rule, visible evidence flags, skipped-update reason for transcript viewers, and the idle fallback reason when no rule matched.
...
A blocked agent makes its pane, tab, and workspace look blocked. A parent waiting on subagents uses the hollow yellow `waiting on agents` state. A working agent makes the workspace look active. A done agent stays visible until you view it. If a working agent misses a declared deadline or goes silent without a declared wait, or a subagent wait exceeds its quiet budget, the server derives `stale`; it is shown separately from a plain-shell `unknown` state until fresh activity or a report resolves it.
