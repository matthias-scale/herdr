...
```bash
created=$(herdr workspace create --cwd ~/project --label api \
  --ticket SCA-42 --pr https://github.com/acme/api/pull/42 \
  --branch feat/api --role ship --active-owner --no-focus)
...
Pass `--pr` with `--role baseline|repair|ship|review` to make the pane's work binding authoritative instead of waiting for Git inference. `--active-owner` requests ownership. Herdr permits one live owner per PR; another run on the same PR is returned as history with `active_owner: false`. Parallel review panes remain attached, and they show the PR as un-owned if the owner pane exits.
...
`idle` means the agent is ready for input and its pane has been opened in the focused Herdr UI. `done` is the same underlying idle state after background work finishes, until that agent's pane is opened or `pane focus` / `agent focus` targets it. Opening another pane in the same tab and reading through the CLI do not mark it seen. `blocked` means Herdr recognized an approval or question UI. `unknown` means an agent is present but Herdr cannot classify its lifecycle confidently; it does not prove successful completion. Use exact `--until` states when that distinction matters.
