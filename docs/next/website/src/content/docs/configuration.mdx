...
The sidebar is the main Herdr dashboard. Its default `spaces` ordering is one workspace tree: each workspace keeps its roll-up row, every tab is shown even when it has no detected agent, and each tab exposes panes in layout order. Linked-worktree and agent disclosure are independent. Compact and mobile navigation preserve the same ownership tree. Selection uses foreground weight rather than a filled row; terminal widgets do not provide a portable font-size control, so sidebar emphasis is limited to spacing, bold text, and glyphs.
...
multiple panes. That row rolls up pane state as `working`, `blocked`, or `done`;
opening a completed tab clears `done` without moving it. Activity age is
supplementary only and never replaces an unseen `done` state.
...
immediately after the window title as `N >_`. The count is summed across panes
owned by that tab, omitted when zero or unsupported, and never changes the
window's lifecycle status or its position. The latest communication age remains
right-aligned.
...
Sidebar row settings affect only the expanded desktop sidebar. Collapsed and mobile views keep compact renderings of the same ownership and disclosure tree.
