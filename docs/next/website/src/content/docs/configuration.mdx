...
Desktop layouts show a right-aligned, full-width status row above the tab bar. Its only visible fields are Git branch, device name, CPU, then memory; the left side stays blank. The literal `CPU` and `MEM` segments stay visible at desktop widths; metrics use percent and GiB and show `CPU --%` or `MEM --/-- GiB` when a sample is unavailable. The row does not display a folder, Herdr version, session, workspace, tab, or pane identity or sample network details or usernames, and it does not show battery, date, or time. As the terminal narrows, branch then device name disappear.
