...
A pane also settles when its agent reads Done and stays quiet for `settle_done_after_minutes`, so a finished thread stops spending context instead of running until reaping closes it. This trigger only applies to panes Herdr can resume into their native agent session; a pane with no resume plan keeps its old path into Recently done and reaping.

The three triggers are independent. Set `auto_settle_finished = false` to stop settling panes whose linked work finished, `auto_settle_inactive = false` to stop settling panes that only went quiet, and `auto_settle_done = false` to stop settling finished agents. All three are on by default and are toggles in **Settings → General**.
...
`prefix+alt+c` opens the same project list as a spawn target: it starts a new
