...
The sidebar is the main Herdr dashboard. Its default `spaces` ordering is one repository tree: each repository and its linked worktrees share one Space row, and every tab appears once as a direct window child even when it has no detected agent or owns multiple panes. Compact and mobile navigation preserve the same ownership tree. Selection uses foreground weight rather than a filled row; terminal widgets do not provide a portable font-size control, so sidebar emphasis is limited to spacing, bold text, and glyphs.

`ui.agent_panel_sort = "priority"` remains accepted for compatibility and the legacy indexed-Agent projection, but the visible sidebar always keeps canonical workspace, tab, and pane order. Lifecycle changes never move identity rows. A transient `agent.view.set` query also leaves the ownership tree unchanged; its compatibility behavior is documented under [Agent view queries](/docs/socket-api/#agent-view-queries). Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, ordering, mouse behavior, pane borders, and other presentation settings.

Each expanded tab shows its persisted title once on one row, even when it owns
multiple panes. Agent-backed rows roll up pane state as `working`, `blocked`, or
`done`; agentless rows omit lifecycle status. Opening a completed tab clears
`done` without moving it. Activity age is supplementary only and never replaces
an unseen `done` state.
...
The visible sidebar is always the Spaces tree. A repository and its linked
...
state icon, state text, separator, and its single title. A tab with several
panes still has one row.
...
configuration does not fail to load, but they do not create a separate Agents
view or child rows in the visible sidebar. Their accepted tokens are:
...
Agentless, unsupported, mixed-provider, and exited-agent rows omit the suffix.
