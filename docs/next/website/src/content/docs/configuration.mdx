...
The visible sidebar is always the Spaces tree. Space roots use
`ui.sidebar.spaces`; every disclosed tab/window below them uses one fixed line:
state icon, state text, separator, and its single title. A tab with several
panes still has one row, and model, agent, and pane subtitles are not rendered.

The Space root layout remains configurable:
...
and consecutive indented worktree children remain packed as one group. Legacy
Agent row declarations remain independently parseable and default to `0`.
