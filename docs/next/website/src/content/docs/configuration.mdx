...
The sidebar is the main Herdr dashboard. Its default `spaces` ordering is one workspace tree: each workspace keeps its roll-up row and can disclose its detected agents underneath. Linked-worktree and agent disclosure are independent. Compact and mobile navigation preserve the same ownership tree.

Set `ui.agent_panel_sort = "priority"` to replace the tree with one flat attention-ordered Agent projection. A transient `agent.view.set` query likewise replaces it with one labeled filtered projection. Opening the workspace picker temporarily restores the Spaces tree without clearing either projection. Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, ordering, mouse behavior, pane borders, and other presentation settings.

Each expanded Agent window row ends with a compact activity age. Working windows show the duration of the current active interval; idle windows show the time since their latest active interval ended. The fixed-width field refreshes only when its displayed value changes, so labels stay aligned, and it disappears before window context is elided at narrow sidebar widths.
