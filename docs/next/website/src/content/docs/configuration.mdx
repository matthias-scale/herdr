...
The two triggers are independent. Set `auto_settle_finished = false` to stop settling panes whose linked work finished, and `auto_settle_inactive = false` to stop settling panes that only went quiet. Both are on by default, and both are toggles in **Settings → General**.
