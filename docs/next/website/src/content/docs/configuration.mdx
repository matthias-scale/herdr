...
`row_gap` controls blank terminal rows between Space roots. Legacy Agent and
Space values remain independently parseable. It defaults to `0`, which packs
entries together; set it to `1` to restore the previous spacing. Consecutive
indented worktree children remain packed as one Space group.
