...
When a settled pane has a native resume plan, Herdr can stop its process and offer Resume. Without one, Herdr keeps the runtime live and clicking its Settled row focuses that pane directly. These live settled panes remain eligible for automatic reaping on `reap_done_after_minutes`; resumable settled panes remain exempt.
