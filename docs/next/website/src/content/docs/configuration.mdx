...
`ui.agent_panel_sort = "priority"` remains accepted for compatibility and attention summaries, but the final sidebar always keeps canonical workspace, tab, and pane order. Lifecycle changes never move identity rows. A transient `agent.view.set` query likewise keeps the ownership projection. Search `ui.` in the [Config reference](/docs/config-reference/) for sizing, collapsed mode, ordering, mouse behavior, pane borders, and other presentation settings.
...
Disabling the row also stops the background metric sampler, so nothing is collected while it is hidden.
