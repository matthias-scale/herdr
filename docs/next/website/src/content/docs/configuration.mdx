...
Desktop layouts show a full-width status row above the tab bar with the focused session's directory and Git branch, network details, memory, CPU, battery, date, and time. Segments are elided from the least important side first when the terminal is too narrow.
