...
Settings → Source control shows these keys. Both defaults keep Herdr's previous
behaviour:

```toml
[source_control]
# Model exported to the Commit action as HERDR_COMMIT_MESSAGE_MODEL, so a
# prepare-commit-msg hook can draft the message with it. Empty, the default,
# runs `git commit` unchanged. Only bare model tokens are exported.
