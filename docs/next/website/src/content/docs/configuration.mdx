...
Each expanded tab shows its persisted title once, followed by pane rows. Pane state remains explicit (`working`, `blocked`, or `done`); opening a completed pane clears `done` without moving its tab. Activity age is supplementary only and never replaces an unseen `done` state.
