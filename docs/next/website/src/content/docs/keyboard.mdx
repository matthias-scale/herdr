...
| Action | Key |
| --- | --- |
| Open ticket or pull request | `prefix+u` |
| Copy ticket or pull request URL | `prefix+shift+u` |
| Copy ticket ID | `prefix+ctrl+u` |
| Copy pull request URL | `prefix+alt+u` |
| Copy preview URL | `prefix+ctrl+shift+u` |

These actions use only the focused pane. A ticket opens its canonical Linear URL; otherwise Herdr uses a validated pull request URL. The granular copy actions copy the bare ticket ID, pull request URL, or first effective Vercel preview URL. If the requested item is absent, Herdr shows a notice and does nothing.
