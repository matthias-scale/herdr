...
next_window = "prefix+shift+n" # optional, cycles tabs across spaces
previous_window = "prefix+shift+p" # optional, cycles tabs across spaces
split_horizontal = ["prefix+minus", "ctrl+alt+shift+d"]
zoom = ["prefix+z", "ctrl+alt+z"]
```

The four split directions have collision-free defaults: `prefix+v` is right, `prefix+shift+v` is left, `prefix+minus` is down, and `prefix+shift+minus` is up. The legacy `split_vertical` and `split_horizontal` names remain right and down. `next_window` and `previous_window` remain optional; they cycle every tab in canonical workspace/tab order, including tabs without agents.
