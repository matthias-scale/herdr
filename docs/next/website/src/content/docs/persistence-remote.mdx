...
`self_name` identifies agents owned by this server. It defaults to the local
fleet host name, then the system hostname when no local host is configured.
Agent references use `host::agent`, for example `workbox::w1:p1`.
