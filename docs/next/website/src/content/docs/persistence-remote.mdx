...
per host. Clicking one attaches to that host and focuses the agent you clicked,
so the session opens on its pane. A host that is already attached is focused
rather than dialled again, so repeated clicks return to the same pane.
