...
## Fleet hosts in the dock

Add `[[remote.fleet.hosts]]` entries to make them available in the Hosts dock
surface. Herdr refreshes their reachability, version, and agent inventory in the
background. Select a reachable remote host to open `herdr --remote` in a new
local pane.
...
`self_name` identifies agents owned by this server. It defaults to the system
hostname, so set it when the configured fleet name should differ from that
hostname. Agent references use `host::agent`, for example `workbox::w1:p1`.

The optional `session` value adds `--session` when the host opens. Hosts marked
`local = true` use the local API socket for inventory reads and never poll over SSH.
An empty Hosts surface points back to `[[remote.fleet.hosts]]`.

Fleet agents also appear in the sidebar, grouped under one collapsible header
per host. Clicking one opens that single remote agent in a local pane, using a
direct terminal attach over SSH rather than a nested Herdr session on the remote
host. An agent that is already attached is focused rather than dialled again, so
repeated clicks return to the same pane. The Hosts dock still opens the whole
remote session.
