...
pub(crate) fn install(content: &str, settings_path: &Path, hook_path: &Path) -> io::Result<String> {
    install_inner(content, settings_path, hook_path, false)
}

pub(crate) fn install_with_title(
    content: &str,
    settings_path: &Path,
    hook_path: &Path,
) -> io::Result<String> {
    install_inner(content, settings_path, hook_path, true)
}
    ...
    if kind == EditKind::Install && !canonical_preserved {
        match hooks.get("SessionStart") {
            Some(property) => {
                ...
                if !include_title_hook && direct_children_are_compact(&session_start.children()) {
                    let updated =
            ...
            None if !include_title_hook && direct_children_are_compact(&hooks.children()) => {
                let updated =
...
fn title_hook_input(hook_path: &Path) -> CstInputValue {
    let command = hook_command(hook_path, Some("title"));
    json!({
        matcher: "*",
        hooks: [{
            "type": "command",
