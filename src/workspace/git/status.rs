...
pub(crate) fn git_status_snapshot_for_cwd_with_demand_and_program(
    ...
    let Some(fingerprint) = fingerprint(repository_context, true, deadline, git_program) else {
        return (
...
fn fingerprint(
    ...
    let head = read_head_identity(&repo.0, deadline, git_program)?;
    let upstream = match &head {
        GitHeadIdentity::Branch { short_name, .. } if include_upstream => {
            read_upstream_identity(&mut repo, short_name, deadline, git_program)
        }
