...
impl SessionWriter {
    ...
    pub(crate) fn save(
        ...
        let result = self.preserve_unloaded().and_then(|()| {
            ...
            super::io::save_to_paths(&self.path, &history_path, snapshot, history)
        });
...
mod tests {
    ...
    fn snapshot_survives_exit_bursts_clears_and_writer_restarts() {
        ...
        writer.save(&original, None).unwrap();
        ...
        for i in 0..100 {
            ...
            writer.save(&shrinking, None).unwrap();
    ...
    fn snapshot_history_is_bounded_and_does_not_rotate_identical_layouts() {
        ...
        writer.save(&snapshot(), None).unwrap();
    ...
    fn snapshot_cadence_recovers_after_clock_rollback_and_restart() {
        ...
        writer.save(&changed, None).unwrap();
    ...
    fn repeated_failed_saves_do_not_replace_a_completed_recovery_copy() {
        ...
        super::super::io::fail_next_json_commits_for_test(2);
        writer.save(&snapshot(), None).unwrap_err();
        writer.save(&snapshot(), None).unwrap_err();
