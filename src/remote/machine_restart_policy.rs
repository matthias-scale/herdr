...
pub(super) fn remote_server_restart_reason(
    ...
    if endpoint_protocol_generation != Some(crate::remote::ENDPOINT_PROTOCOL_GENERATION) {
...
mod tests {
    ...
    fn pre_floor_server_requires_update() {
        assert_eq!(
            remote_server_restart_reason(None, true, true, false, false),
    ...
    fn compatible_server_can_keep_running() {
        assert_eq!(
            remote_server_restart_reason(
                Some(crate::remote::ENDPOINT_PROTOCOL_GENERATION),
                true,
                false,
                false,
                false
