...
use super::{
    machine_args::*, machine_process::wait_with_output_timeout, machine_restart_policy::*,
    shell_quote,
};
...
pub(crate) fn prepare_saved_ssh(
    ...
    // Saved machines must never fall back to interactive or unverified SSH.
    ssh.noninteractive = true;
    // The fork keeps saved-machine API routing alongside its existing remote-focus
    // client; it does not require the upstream client-owned shell surface here.
    let prepared = prepare_remote_herdr(&ssh, false, false)?;
    ensure_remote_server_ready(
        &ssh,
    ...
    match remote_server_status(&ssh, &prepared.remote_herdr, false)? {
...
fn remote_server_status(
    ...
    if require_surface_interest
        && matches!(
            status,
            RemoteServerStatus::Running {
                endpoint_protocol_generation: Some(crate::remote::ENDPOINT_PROTOCOL_GENERATION),
...
impl RemoteClientStatusJson {
    fn supports_endpoint_requirement(
        ...
        self.endpoint_protocol_generation == Some(crate::remote::ENDPOINT_PROTOCOL_GENERATION)
            && (!platform.is_windows() || self.remote_host_bridge)
            && (!require_surface_interest
                || [
                    crate::remote::SURFACE_INTEREST_CAPABILITY,
                    crate::remote::PRESENTATION_EFFECTS_FENCE_CAPABILITY,
                    crate::remote::HEALTH_CHECK_CAPABILITY,
                ]
                .iter()
                .all(|required| {
    ...
    fn saved_machine_compatibility_uses_capabilities_not_release_or_private_protocol() {
        ...
        let mut status = RemoteClientStatusJson {
            ...
            endpoint_capabilities: vec![
                crate::remote::SURFACE_INTEREST_CAPABILITY.into(),
                crate::remote::PRESENTATION_EFFECTS_FENCE_CAPABILITY.into(),
                crate::remote::HEALTH_CHECK_CAPABILITY.into(),
            ],
