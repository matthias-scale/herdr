...
impl SavedSshApiBridge {
    pub(crate) fn start(
        ...
        let metadata = match cached {
            ...
            None => {
                let metadata = super::machine_attach::discover_remote_api_metadata(&ssh, session)?;
                metadata_cache.store(&metadata);
                metadata
            }
        };
        let command = super::machine_attach::cached_remote_api_command(&metadata, session);
    ...
    pub(crate) fn stale_metadata_failure(error: &io::Error) -> bool {
        error
            .to_string()
            .contains(super::machine_attach::STALE_API_METADATA)
    }
}
