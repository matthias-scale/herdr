...
mod tests {
    ...
    impl Default for ParsedLinkHarness {
        fn default() -> Self {
            ...
            terminal.set_parsed_output_enabled(true);
            Self {
                gate,
    ...
    impl ParsedLinkHarness {
        fn observe_chunk(&self, bytes: &[u8]) {
            self.terminal.lock().expect("Ghostty terminal").write(bytes);
        }
    }
    ...
    fn reader_visible_link_oracle(bytes: &[u8]) -> CollectedLinks {
        let parsed = Arc::new(Mutex::new((Vec::new(), Vec::new())));
        let parsed_for_callback = parsed.clone();
        let mut terminal = crate::ghostty::Terminal::new(240, 24, 0).expect("Ghostty terminal");
        terminal.set_parsed_output_callback(move |event| {
            let mut parsed = parsed_for_callback.lock().expect("parsed output collector");
            match event {
                crate::ghostty::ParsedOutput::Text(bytes) => parsed.0.extend_from_slice(bytes),
                crate::ghostty::ParsedOutput::Separator => parsed.0.push(b'\n'),
                crate::ghostty::ParsedOutput::Hyperlink(bytes) => {
                    parsed
                        .1
                        .extend(extract_urls(&String::from_utf8_lossy(bytes)));
                }
            }
        });
        terminal.set_parsed_output_enabled(true);
        terminal.write(bytes);
        terminal.set_parsed_output_enabled(false);

        let parsed = parsed.lock().expect("parsed output");
        let mut output_urls = extract_urls(&String::from_utf8_lossy(&parsed.0));
        let mut osc8_urls = parsed.1.clone();
        output_urls.sort_unstable();
        output_urls.dedup();
        osc8_urls.sort_unstable();
        osc8_urls.dedup();
    ...
    fn scan_with_generated_splits(bytes: &[u8], seed: u64) -> CollectedLinks {
        ...
        while offset < bytes.len() {
            ...
            terminal.write(&bytes[offset..end]);
            offset = end;
            if random & 3 == 0 {
    ...
    fn round_10_generated_terminal_interleavings_match_reader_visible_oracle() {
        for seed in 0..32 {
            ...
            assert_eq!(chunked.osc8_urls.len(), 4, "OSC 8 sources, seed {seed}");
        }
    }
